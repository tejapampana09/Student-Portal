...
export async function GET(req: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(`${origin}/dashboard?oauth_error=missing_params`);
  }
  ...
  try {
    ...
    // Atomically find and consume single-use state nonce
    ...
    if (!stateDoc) {
      console.warn("Rejected OAuth callback with invalid or expired state nonce.");
      return NextResponse.redirect(`${origin}/dashboard?oauth_error=invalid_or_expired_state`);
    }

    const username = stateDoc.username;
    const returnTo = stateDoc.returnTo || "/dashboard";
    ...
    if (!tokens.refresh_token) {
      console.warn("No refresh token received during OAuth token exchange.");
    }
    ...
    // Save under unified googleOAuth and mirror for backward-compatible readers
    ...
    console.error("Error during secure Google OAuth callback:", error?.message || error);
    return NextResponse.redirect(`${origin}/dashboard?oauth_error=callback_failed`);
  }
}
