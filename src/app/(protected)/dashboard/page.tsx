...
} from "@/components/page/dashboard/CustomizeDashboardModal";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("srmap_dashboard_widgets");
      if (saved) {
        setWidgetsConfig((prev) => ({ ...prev, ...JSON.parse(saved) }));
      }
    } catch {}
  }, []);

  const handleWidgetsChange = (newConfig: DashboardWidgetsConfig) => {
    setWidgetsConfig(newConfig);
    try {
      localStorage.setItem("srmap_dashboard_widgets", JSON.stringify(newConfig));
    } catch {}
  };
  ...
  // Count active widgets in middle row to calculate responsive grid column spans
  const middleActiveCount = [
    widgetsConfig.cgpaCard,
    widgetsConfig.timetable,
    widgetsConfig.courseBreakdown,
  ].filter(Boolean).length;

  const getMiddleColSpan = () => {
    if (middleActiveCount === 1) return "md:col-span-12";
    if (middleActiveCount === 2) return "md:col-span-6";
    ...
    <div className="flex flex-col gap-4 pb-8 max-w-7xl mx-auto w-full">
      ...
      {(widgetsConfig.attendanceRing || widgetsConfig.upcomingClass) && (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          {widgetsConfig.attendanceRing && (
            <div
              className={`${
                widgetsConfig.upcomingClass ? "md:col-span-5" : "md:col-span-12"
              } glass-card rounded-3xl p-6 flex flex-col justify-between border border-white/10 shadow-lg relative overflow-hidden`}
            >
          ...
          {widgetsConfig.upcomingClass && (
            <div
              className={`${
                widgetsConfig.attendanceRing ? "md:col-span-7" : "md:col-span-12"
              } glass-card rounded-3xl p-6 sm:p-7 flex flex-col justify-between border border-white/10 shadow-lg relative overflow-hidden`}
            >
              <div>
      ...
      {(widgetsConfig.cgpaCard || widgetsConfig.timetable || widgetsConfig.courseBreakdown) && (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          {/* 3. Current CGPA Widget */}
          {widgetsConfig.cgpaCard && (
            <div className={`${getMiddleColSpan()} glass-card rounded-3xl p-6 flex flex-col justify-between border border-white/10 shadow-lg`}>
              <div>
                <div className="flex items-center justify-between mb-3">
      ...
      {(widgetsConfig.upcomingHolidays || widgetsConfig.studentEmails) && (
        <div
          className={`grid grid-cols-1 ${
            widgetsConfig.upcomingHolidays && widgetsConfig.studentEmails ? "md:grid-cols-2" : "md:grid-cols-1"
          } gap-4 mt-2`}
        >
          {/* 6. Upcoming Holidays & Long Weekends Widget */}
          {widgetsConfig.upcomingHolidays && <UpcomingHolidaysCard />}

          {/* 7. Student Email & Placement Notices Widget */}
          {widgetsConfig.studentEmails && <StudentEmailCard />}
        </div>
      )}
